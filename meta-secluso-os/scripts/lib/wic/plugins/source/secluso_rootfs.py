...
#   upstream sha256: 143d42e142cc2f5a50b8302b91b09968b6f20e07a297239382565f50ee85919b
# END MODIFIED AREA 1.
...
class RootfsPlugin(SourcePlugin):
    ...
    # Keep OE-Core's implementation but register it under a project-local source name so the WKS file can opt into the reproducible variant.
    name = 'secluso_rootfs'
    # END MODIFIED AREA 2.
    ...
    def do_prepare_partition(cls, part, source_params, cr, cr_workdir,
        ...
        # MODIFIED AREA 3: STABILIZE EXT4 WRITE ACCOUNTING.
        # OE-Core's ext4 creation leaves s_kbytes_written dependent on the number of host-side writes performed while mkfs/fsck/debugfs populate the image.
        # It is not actual file content we need.
        # Reset it after OE-Core has prepared the rootfs so the superblock checksum is stable across build hosts
        if part.fstype == "ext4":
            debugfs_script_path = os.path.join(cr_workdir,
                                               "secluso-debugfs-script-%s" %
                                               part.lineno)
            with open(debugfs_script_path, "w") as f:
                f.write("set_super_value kbytes_written 0\n")
